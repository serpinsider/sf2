...
export default function CommercialStepWizard({ onFormExpand }: CommercialStepWizardProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('https://formspree.io/f/xdkoqvgz', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          business: 'San Jose Maids', 
          ...
          confirmationNumber,
          _subject: `San Jose Maids - Commercial Cleaning Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`, 
      ...
      case 3:
        return (
          <div className="space-y-6">
            ...
            <p className="text-[10px] sm:text-xs text-white/60 text-center leading-relaxed">By submitting, you agree to receive communications from San Jose Maids regarding your quote request.</p>
          </div>
        );
      default: return null;
    }
